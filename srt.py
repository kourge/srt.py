...
class Timecode:
  def __init__(self, time):
    if isinstance(time, (int, long)):
      ...
      self.ms = sum([int(seconds, 10) * 1000,
                     int(minutes, 10) * 1000 * 60,
                     int(hours, 10) * 1000 * 60 * 60,
                     int(milliseconds, 10)]) * sign
  ...
  def stringify(total):
    ...
    negative = False
    if total < 0: total, negative = (-total, True)

    milliseconds = total % 1000; total /= 1000
    seconds = total % 60; total /= 60
    minutes = total % 60; total /= 60
    hours = total

    formatted = "%02d:%02d:%02d,%03d" % (hours, minutes, seconds, milliseconds)
    if not negative: return formatted
...
def main(argv=None):
  ...
  try:
    try:
      ...
      if len(argv) < 2: argv.append("help")
